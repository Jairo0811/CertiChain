...
const BRAND_ISOTYPE = readFileSync(
  new URL("../assets/branding/certichain-isotipo-header.jpg", import.meta.url),
);
...
function pushHexagon(
  ...
  commands.push("q", `${color.join(" ")} RG`, "0.55 w");
  commands.push(`${points[0][0].toFixed(2)} ${points[0][1].toFixed(2)} m`);
...
function pushBlockchainPattern(commands: string[]): void {
  ...
  const nodes = [
    [530, 420], [570, 448], [612, 420], [653, 447], [698, 420], [742, 448], [785, 420],
    [548, 265], [594, 286], [640, 263], [688, 286], [734, 263], [778, 286],
  ] as const;

  for (let index = 0; index < 6; index += 1) {
    pushLine(commands, nodes[index][0], nodes[index][1], nodes[index + 1][0], nodes[index + 1][1], cyan, 0.55);
  }
  for (let index = 7; index < nodes.length - 1; index += 1) {
    pushLine(commands, nodes[index][0], nodes[index][1], nodes[index + 1][0], nodes[index + 1][1], violet, 0.55);
  }

  for (const [x, y] of nodes) {
    pushRect(commands, x - 1.7, y - 1.7, 3.4, 3.4, { fill: [0.76, 0.86, 0.98] });
  }
...
function pushQrCode(commands: string[], value: string, x: number, y: number, size: number): void {
  ...
  const quietZone = 4;
  const modulesAcross = matrix.length + quietZone * 2;
  const moduleSize = size / modulesAcross;
...
export function buildCertificatePdf(certificate: CertificatePdfInput | CertificateRecord): Buffer {
  ...
  // Main academic statement.
  pushText(commands, "CERTIFICADO ACADEMICO VERIFICABLE", 56, 443, 10, {
    bold: true,
  ...
  pushText(commands, "ha recibido la credencial academica", 56, 329, 11.5, {
    color: [0.35, 0.4, 0.52],
  });
  ...
  pushText(commands, `Fecha de emision · ${formatIssuedDate(certificate.issuedAt)}`, 56, 216, 9.5, {
    color: [0.39, 0.45, 0.57],
  });

  // Trust-layer panel: blockchain identity without pretending that mutable state is embedded in the PDF.
  ...
  pushText(commands, "WALLET / IDENTIDAD TECNICA", 76, 113, 7.5, {
    bold: true,
  ...
  pushText(commands, "VERIFICACION PUBLICA", 430, 173, 8.5, {
    bold: true,
  ...
  pushText(commands, "El SHA-256 se recupera automaticamente", 430, 91, 7.4, {
    color: [0.63, 0.7, 0.84],
  });
  ...
  pushText(
    commands,
    "El estado es dinamico: vigencia y revocacion se consultan en el Verificador CertiChain.",
    55,
    30,
    8.3,
